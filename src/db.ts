/**
 * SQLite database access layer for the BaFin MCP server.
 *
 * Schema mirrors the BaFin regulatory structure:
 *   - sourcebooks        — BaFin Rundschreiben and Mindestanforderungen (MaRisk, BAIT, VAIT, etc.)
 *   - provisions         — individual sections, requirements, and guidance within each circular
 *   - enforcement_actions — BaFin Massnahmen: Bussgeld, Untersagungen, and Anordnungen
 *
 ...
 *
 * FTS5 treats hyphens as the minus (exclusion) operator and interprets many
 * punctuation characters as syntax. Wrap the raw query in double quotes so
 * the entire string is treated as a phrase search, escaping any embedded
 * double quotes. If the caller already provides FTS5 syntax (starts with a
 * double quote), pass it through unchanged.
 */
...
export function searchProvisions(opts: SearchProvisionsOptions): Provision[] {
  ...
  return db
    ...
    .all({ query: ftsQuery, limit }) as Provision[];
}
...
export function getProvision(
  ...
  return (
    (db
      .prepare(
        "SELECT * FROM provisions WHERE sourcebook_id = ? AND UPPER(reference) LIKE ? LIMIT 1",
      )
      .get(sourcebook.toUpperCase(), `${upper}%`) as Provision | undefined) ??
    null
  );
}
...
export function checkProvisionCurrency(reference: string): {
  ...
  const row = db
    .prepare(
      "SELECT reference, status, effective_date FROM provisions WHERE UPPER(reference) = ? LIMIT 1",
    )
...
export function searchEnforcement(
  ...
  if (opts.action_type) {
    return db
      ...
      .all({ query: ftsQuery, action_type: opts.action_type, limit }) as EnforcementAction[];
  }
  ...
  return db
    ...
    .all({ query: ftsQuery, limit }) as EnforcementAction[];
}
