# ─────────────────────────────────────────────────────────────────────────────
# German Financial Regulation MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t german-financial-regulation-mcp .
# Run:    docker run --rm -p 3000:3000 german-financial-regulation-mcp
#
# The image bakes /app/data/bafin.db at build time. Override BAFIN_DB_PATH
...
ENV BAFIN_DB_PATH=/app/data/bafin.db
...
# Bake the pre-built database into the image so /app/data/bafin.db resolves
# at runtime without a bind mount. This matches the working pattern in
# cypriot-data-protection-mcp's GHCR image.
#
# The explicit `data/bafin.db` reference is required — `.github/workflows/
